...
class Employee(models.Model):
    ...
    job_id = fields.Many2one('gangwei', string='Job Title')
    ...
    bianhao = fields.Char('编号')
    jiguan = fields.Char('籍贯')
    minzu = fields.Char('民族')
    shengao = fields.Char('身高（cm）')
    tizhong = fields.Char('体重（kg）')
    zhengzhimianmao = fields.Selection([('dangyuan', '党员'), ('tuanyuan', '团员'), ('qunzhong', '群众')], string='政治面貌')
    hukousuozaidi = fields.Char('户口所在地')
    jiankangzhuangkuang = fields.Char('健康状况')
    shifoupeidaiyanjing = fields.Selection([('yes', '是'), ('no', '否')], string='是否佩戴眼镜')
    zuigaoxueli = fields.Selection([('boshi', '博士'), ('shuoshi', '硕士'), ('benke', '本科'), ('gaozhongjiyixia', '高中及以下')],
                                   string='最高学历')
    biyeyuanxiao = fields.Char('毕业院校')
    waiyushuiping = fields.Char('外语水平')
    youbian = fields.Integer('邮编')
    shixifrom = fields.Date('实习期从')
    shixito = fields.Date('实习期到')
    xianrenzhizhuangkuang = fields.Selection([('shiyong', '试用'), ('zhuanzheng', '转正')], string='现任职状况')
    shifoutijianjitijianjieguo = fields.Char('是否体检及体检结果')
    shiyezheng = fields.Char('失业证')
    xuexijingli = fields.Text('学习经历')
    peixunjingli = fields.Text('培训经历')
    gongzuojingli = fields.Text('工作经历')
    jiatingchengyuan = fields.Text('家庭成员')
    samedepartment_uid = fields.Many2many('res.users')
    ...
    state = fields.Selection([('draft', '个人信息创建'), ('confirmed', '人力专员审批'), ('confirmedt', '部门经理审批'), ('done', '审批完成')],
    ...
    @api.multi
    def get_current_user_department_id(self, uid):

        res2_id = self.env.get('resource.resource').search([('resource_type', '=', 'user'), ('user_id', '=', uid)])  # 获取关联表ID

        depart_id = self.env.get('hr.employee').browse([('resource_id', '=', res2_id[0])]).department_id  # 获取员工表的部门ID
...
class Department(models.Model):
    ...
    bianhao = fields.Char('部门编号')
    bumenjieshao = fields.Text('部门介绍')
    bumen_uid = fields.Many2many('res.users')
    state = fields.Selection(
        [('draft', '部门经理创建'), ('confirmed', '分管副总审批'), ('confirmedt', '总经理审批'), ('done', '审批完成')],
